...
int main(int argc, char **argv) {
  ...
  if (test_args == 0) {
    ...
    memObjects[0] = clCreateBuffer(context, CL_MEM_READ_WRITE,
                                   sizeof(float) * ARRAY_SIZE, NULL, &errNum);
  ...
  // hostPtr = (float
  // *)clEnqueueMapBuffer(commandQueue,memObjects[2],CL_TRUE,CL_MAP_WRITE,0,sizeof(float)
  // * ARRAY_SIZE,0, NULL, NULL, &errNum);
  if (test_args == 0) {
    printf("test clCreateBuffer and read\n");
    errNum =
    ...
    hostPtr = (float *)clEnqueueMapBuffer(
        commandQueue, memObjects[0], CL_TRUE, CL_MAP_READ, 0,
        sizeof(float) * ARRAY_SIZE, 0, NULL, NULL, &errNum);
  } else if (test_args == 2) {
    printf("test svm\n");
